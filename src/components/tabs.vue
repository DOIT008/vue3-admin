<template>
  <div>
    <el-tabs v-model="activeName" type="card" editable class="demo-tabs" @tab-click="handleClick"
      @tab-remove="tabRemove">
      <el-tab-pane :label="item.title" v-for="(item) in tabList" :key="item.path" :name="item.path"></el-tab-pane>
    </el-tabs>
  </div>
</template>
  ...
  setup() {
    ...
    function updateTabList(route: any) {
      store.$patch((store) => {
        ...
        let currentRoute = store.tabList.find(item => item.path === route.path);
        if (currentRoute) {
    ...
    function tabRemove(tabName: TabPaneName) {
      ...
      let index = store.tabList.findIndex(item => item.path === tabName)
      // 如果tablist大于1
